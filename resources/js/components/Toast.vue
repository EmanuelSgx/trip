<template>
    ...
    >
        <div
            v-if="show"
            class="pointer-events-auto fixed right-4 top-4 z-50 w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5"
        >
            <div class="p-4">
                    ...
                    <div class="ml-3 w-0 flex-1 pt-0.5">
                        ...
                        <div v-if="actions && actions.length > 0" class="mt-3 flex space-x-2">
                            <button
                                ...
                                class="inline-flex transform items-center rounded-md border border-transparent px-3 py-1.5 text-xs font-medium shadow-sm transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2"
                                :class="{
                                    ...
                                    'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 focus:ring-indigo-500':
                                        action.type === 'secondary' || !action.type,
                                }"
                            >
                                <svg v-if="action.type === 'success'" class="mr-1 h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                </svg>
                                <svg v-else-if="action.type === 'error'" class="mr-1 h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
